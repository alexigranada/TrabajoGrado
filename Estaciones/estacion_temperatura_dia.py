...
ruta_max = 'C:/Datos/Estaciones/Temperatura máx diaria.csv' #Ruta del archivo
...
fechaformato = "%d/%m/%Y %H:%M"
estacion_max['Fecha'] = pd.to_datetime(estacion_max['Fecha'],format=fechaformato)
...
#'''
temp_max = df_final_estacion_max['Valor']
temp_min = df_final_estacion_min['Valor']
time_max = df_final_estacion_max['Fecha']
time_min = df_final_estacion_min['Fecha']
temp_media = unir_df['ValorMedio']
time_media = unir_df['Fecha']
...
fig.add_trace(go.Scatter(x=time_min, y=temp_min, mode='lines', name='Temp. Mín', line=dict(color='#636EFA')))
