...
Exploración dato Velocidad del Viento de las estaciones del IDEAM
...
'''Cargamos datos'''
ruta = 'Datos/IDEAM/Estaciones Dagua IDEAM/VA_hora/Vel_viento_SILOE - AUT [26085160].csv'
...
df = df.rename(columns={'Valor': 'W_S (m/s)'})
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=time, y=valor, mode='lines', name='Vel. Viento'))

fig.update_layout(title=f'Velocidad del viento: {name}',
                  title_x= 0.5)
fig.show()

''' Exportar el DataFrame a un archivo CSV '''
title = f'VV_hora_{name}.csv'
